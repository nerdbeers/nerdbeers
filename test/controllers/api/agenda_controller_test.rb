...
class Api::AgendaControllerTest < ActionController::TestCase
  ...
  test "validate standard signature" do
    get :show
    assert_response :success
    assert_equal Mime::JSON, response.content_type
    ...
    validate_sig @agenda, @venue, payload

    #move the troublesome assert to the end... intentionally moved out of common method so we can test perty date
    assert_equal @agenda.meeting_date, payload[:meeting_date]
  end

  test "validate all signature" do
    get :all
    assert_response :success
    assert_equal Mime::JSON, response.content_type
    ...
    validate_sig agenda, @venue, item

    assert_equal agenda.meeting_date.strftime("%b %d, %Y"), item[:perty_date], 'Reminder: Perty date is for auto-suggestion'

    #move the troublesome assert to the end... intentionally moved out of common method so we can test perty date
    assert_equal agenda.meeting_date, item[:meeting_date]

  end

  def validate_sig agenda, venue, payload
